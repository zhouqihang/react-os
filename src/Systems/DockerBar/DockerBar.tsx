...
const DockerBar: FunctionComponent<IDockerBarProps> = (props) => {
  const { apps, activeAppNamespace, className, onClick, ...otherProps } = props;
  ...
  return (
    <section className={`${prefix}-container`} id="os-docker">
      <div className={cn(prefix, className)} {...otherProps}>
        {
          apps.map(app => (
            <DockerItem active={activeAppNamespace.includes(app.namespace)} name={app.name} namespace={app.namespace} icon={app.icon} key={app.namespace} onClick={() => onClick(app)} />
          ))
        }
      </div>
    </section>
